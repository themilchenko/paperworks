...
Теперь сформулируем одну из основных теорем этой работы, утверждающую, что задача \textsc{INSECURE} разрешима в классе \textsf{NP} для любого множества оракульных правил, удовлетворяющего двум условиям. Данная общая теорема будет применена в разделе 4 и разделе 7, чтобы показать, что \textsc{INSECURE} остаётся в \textsf{NP} в присутствии злоумышленника, способного выполнять экспоненцирование Диффи–Хеллмана и использовать коммутативное шифрование с открытым ключом, соответственно.
...
Теорема доказывается следующим образом. Сначала показывается, что недетермированный алгоритм из рисунка 1 является недетермированным полиномиальным алгоритмом и корректно решает задачу \textsc{INSECURE}. Полином \(p\), ограничивающий размер показателей произведений, существует в силу того, что \(L_o\) допускает полиномиальные атаки на показатели произведений. Структура алгоритма такова: на шагах 1 и 2 недетермированно выбирается «маленькая» атака \((\pi,\sigma)\) на протокол \(P\), а на шагах 3 и 4 проверяется, действительно ли \((\pi,\sigma)\) является атакой на \(P\).
...
В разделе 3.1 (см. теорему 3.2) показывается, что следующая задача, далее называемая \emph{задачей вывода (derivation problem)}, решается за полиномиальное время от \(\lVert E,t\rVert\), при условии, что \(\mathsf{OracleRule}\) разрешима в детермированном полиномиальном времени:
\[
...
Покажем, что алгоритм из рисунка 1 работает в недетермированное полиномиальное время и является корректным и полным. Корректность уже была доказана.
...
Остаётся показать, что наш алгоритм выполняется в недетермированное полиномиальное время по величине \(\lVert P\rVert\). Для шагов 1 и 2 это очевидно. Чтобы убедиться в полиномиальности шагов 3 и 4, воспользуемся теоремой 3.2. Пусть $E \;=\;\{\ulcorner S_j\sigma\urcorner \mid j<i\}\;\cup\;\{\ulcorner S_0\urcorner\}$
...
может быть выполнена за детермированное полиномиальное время от \(\lVert E,t\rVert\). По следствию 3.16 имеем
...
По лемме 2.4 из этого следует, что \(\lVert E,t\rVert_{\mathrm{exp}}\) полиномиально ограничено по \(\lVert P\rVert\). Следовательно, шаги 3 и 4 могут быть выполнены за детермированное полиномиальное время по \(\lVert P\rVert\).  
\(\Box\)

\subsection{Решение задачи вывода}

Покажем, что задачу вывода (derivation problem) можно решить за полиномиальное время, при условии, что задача \textsf{OracleRule} разрешима в детермированном полиномиальном времени.
...
$E\;\to_{L_o}\;E,t'$
выполняется за детермированное полиномиальное время от $\|E,t\|$, нетрудно убедиться, что $d_{t}(E)$ вычисляется за полиномиальное время от $\|E,t\|$. 
...
Поскольку каждый шаг вычисления $d_{t}^{i}(E)$ занимает полиномиальное время от $\|E,t\|$, вытекает, что проверка $t\in\mathrm{forge}(E)$ также выполняется за детермированное полиномиальное время.  
\end{proof}
...
$E$ — множество нормализованных термов,
$s$ — нормализованный стандартный неатомарный терм,
а $\delta$ — замена $[s \leftarrow 1]$.
Обозначим $\sigma'=\ulcorner\sigma\delta\urcorner$.
Если не существует стандартного подтерма $t\in E$ такого, что  
...
\textsc{Лемма 3.6.} 
Пусть $t',\,t_1,\dots,t_n,\,t,\,u$ — нормализованные стандартные термы,
...
\textsc{Лемма 3.7.}
Пусть $s$ — стандартный терм, $t$ — нормализованный терм,
...
Тогда существует стандартный подтерм $t'$ терма $t$,
...
(относительно отношения «подтерм»),
...
такой терм $t_x$ корректно определён.
...
термы \(t_j\) являются замкнутыми.
Пусть \(j_0\) — индекс незамкнутого подтерма, т.\,е.\ \(x\in\mathcal V\!\bigl(t_{j_0}\bigr)\).
...
что противоречит предположению. Если оба терма \(\ulcorner t_{j_0}\sigma \urcorner\) и \(\ulcorner t_{j_0}\sigma' \urcorner\)
...
Используя предыдущую лемму, покажем, что любой подтерм подстановки,
соответствующей минимальной атаке,
либо принадлежит спецификации протокола,
либо является подтермом нормализованного сообщения,
...
\(i\in\{1,\dots,k\}\),
а \(s\in \mathcal S\!\bigl(\sigma(x)\bigr)\) — стандартный терм.
...
позволяющих без труда заменять термы внутри вывода.
...
$t'$ — подтерм терма $t$,
терм $t$ выводится из множества $E$,
но при этом $t'$ не является подтермом ни одного элемента $E$,
...
— вывод терма \(t\), причём \(E_0 = E\).
Так как \(t'\) является подтермом элементов \(E_n\),
...
Следующая лемма будет использована в доказательстве Леммы 3.13.
Она позволяет строить специальные выводы, в которых заданный терм
никогда не декомпозируется. Это окажется критически важным,
когда потребуется заменить составные термы атомами в ряде выводов.
...
заменять определённые подтермы, возникающие в подстановке атаки,
на более короткие.
Из предположений текущей леммы следует, что
$s$ выводим из $E$ так, что последним правилом является правило композиции.
Это даёт возможность заменить $s$ на более короткий терм,
...
Пусть
$t\in forge(E,F)$ и
$s\in forge(E)$ — неатомарный терм, причём $s\notin \mathcal S(E)$.
...
связывая подтермы, изначально присутствующие в протоколе~$P$.
Это ключевой шаг, позволяющий ограничить число подтермов
...
Итак, $s$ — неатомарный терм,
не являющийся подтермом $E_N$,
но являющийся подтермом
...
\medskip
\noindent— \textbf{$j<N$.}
Минимальность $N$ означает, что $s$ не является подтермом
...
Но $\sigma'$ получена из $\sigma$ заменой $s$ на терм $1$,
то есть
\(|\sigma'|<|\sigma|\),
что противоречит минимальности атаки $(\pi,\sigma)$.
\end{proof}

\subsection{Ограничение числа подтермов минимальных атак}
